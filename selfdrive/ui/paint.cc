...
// TPMS code added from OPKR
  ...
  } else if ((scene.tpmsPressureFl > 45 && scene.tpmsUnit != 2) || (scene.tpmsPressureFl > 3 && scene.tpmsUnit == 2)) {
  ...
  if ((scene.tpmsPressureFr < 32 && scene.tpmsUnit != 2) || (scene.tpmsPressureFr < 2.2 && scene.tpmsUnit == 2)) {
    ...
    ui_draw_text(s, pos_x+45, pos_y+45, "N/A", 50, COLOR_WHITE_ALPHA(200), "sans-semibold");
  } else if ((scene.tpmsPressureFr > 45 && scene.tpmsUnit != 2) || (scene.tpmsPressureFr > 3 && scene.tpmsUnit == 2)) {
  ...
  } else if ((scene.tpmsPressureRl > 45 && scene.tpmsUnit != 2) || (scene.tpmsPressureRl > 3 && scene.tpmsUnit == 2)) {
  ...
  if ((scene.tpmsPressureRr < 32 && scene.tpmsUnit != 2) || (scene.tpmsPressureRr < 2.2 && scene.tpmsUnit == 2)) {
    ...
    ui_draw_text(s, pos_x+45, pos_y+90, "N/A", 50, COLOR_WHITE_ALPHA(200), "sans-semibold");
  } else if ((scene.tpmsPressureRr > 45 && scene.tpmsUnit != 2) || (scene.tpmsPressureRr > 3 && scene.tpmsUnit == 2)) {
    ui_draw_text(s, pos_x+45, pos_y+90, tpmsFr, font_size, COLOR_RED, "sans-semibold");
  } else {
...
static void bb_ui_draw_measures_left(UIState *s, int bb_x, int bb_y, int bb_w ) {
  ...
  if (0 < scene.gear_step && scene.gear_step < 6) {
    //char val_str[16];
    //char uom_str[6];
    std::string trans_gear_val = "Lv " + std::to_string(int(scene.gear_step));
    std::string gap = "";
    NVGcolor val_color = nvgRGBA(0, 220, 255, 255);
    NVGcolor uom_color2 = COLOR_WHITE_ALPHA(200);
    ...
    bb_ry +=bb_ui_draw_measure(s, trans_gear_val.c_str(), gap.c_str(), "REGEN",
        bb_rx, bb_ry, bb_uom_dx,
        val_color, lab_color, uom_color2,
        value_fontSize, label_fontSize, uom_fontSize, 0);
  }

  if (scene.gear_step > 6) {
    //char val_str[16];
    //char uom_str[6];
    std::string trans_gear_val = "AUTO";
    std::string gap = "";
    NVGcolor val_color = nvgRGBA(0, 220, 255, 255);
    NVGcolor uom_color2 = COLOR_WHITE_ALPHA(200);
     if (scene.cruise_gap == 1) {
    ...
    bb_ry +=bb_ui_draw_measure(s, trans_gear_val.c_str(), gap.c_str(), "REGEN",
        bb_rx, bb_ry, bb_uom_dx,
        val_color, lab_color, uom_color2,
        value_fontSize=22, label_fontSize, uom_fontSize, 2);
  }
   //add Battery SOC
{
    char val_str[16];
    char uom_str[6];
    NVGcolor val_color = COLOR_GREEN_ALPHA(200);
     //show red/orange/yellow Battery state
    if(scene.charge_meter < 70) {
      val_color = COLOR_YELLOW_ALPHA(200);
    }
    if(scene.charge_meter < 40) {
      val_color = COLOR_ORANGE_ALPHA(200);
    }
     if(scene.charge_meter < 20) {
      val_color = COLOR_RED_ALPHA(200);
    }
        // Battery State in percent
    std::string bat_charge_soc = std::to_string(int(scene.charge_meter)) + "%";
    snprintf(val_str, sizeof(val_str), "%.0f", (scene.charge_meter));
    snprintf(uom_str, sizeof(uom_str), "-");
    bb_ry +=bb_ui_draw_measure(s, bat_charge_soc.c_str(), uom_str, "BATTERY",
        bb_rx, bb_ry, bb_uom_dx,
        val_color, lab_color, uom_color,
        value_fontSize=38, label_fontSize, uom_fontSize, 0);
  }
...
static void ui_draw_rpm_animation(UIState *s) {
  ...
  nvgTextAlign(s->vg, NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE);
  ui_draw_text(s, center_x, center_y+110, s->scene.is_metric?"Km/h":"MPH", 65, s->scene.brakeLights?COLOR_RED_ALPHA(200):COLOR_WHITE_ALPHA(200), "sans-semibold");
